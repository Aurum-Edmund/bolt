...
- **Type qualifiers**: `constant` must prefix the type name and may appear only once. Repeating the qualifier triggers diagnostic `BOLT-E2301`, while placing it after the type raises `BOLT-E2303` with guidance to reposition the keyword, even when the misplaced keyword appears inside generic argument lists. Unrecognised qualifiers are rejected with diagnostic `BOLT-E2302`, and legacy `const` usage surfaces the same diagnostic with a fix-it message pointing to `constant`. Pointer and reference builtins must specify exactly one target type argument; missing or extra arguments emit diagnostics `BOLT-E2304` and `BOLT-E2305` respectively with guidance to supply a single target type.
- **Live qualifiers**: `live` may prefix a return, parameter, or blueprint field type exactly once. Missing concrete type information raises `BOLT-E2217` with guidance that the qualifier must reference a concrete type, duplicate prefixes surface `BOLT-E2218` with instructions to remove the extra keyword, and misplaced tokens after the type (including inside generic arguments) raise `BOLT-E2219` with a fix-it to move `live` ahead of the type name.
- **Canonical type text**: The binder produces a normalized string for every type reference that trims extraneous whitespace, standardises generic separators, and preserves qualifiers. MIR lowering propagates this canonical text so downstream passes can depend on consistent formatting for comparisons and diagnostics.
