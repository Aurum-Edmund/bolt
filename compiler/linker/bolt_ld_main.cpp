...
namespace linker
{
    ...
    static int runLinker(const CommandLineOptions& options)
    {
        ...
        auto logPath = [](const char* label, const std::string& value) {
            if (!value.empty())
            {
        ...
        logPath("[bolt-ld] output: ", options.outputPath);
        logPath("[bolt-ld] linker script: ", options.linkerScriptPath);
        logPath("[bolt-ld] import bundle: ", options.importBundlePath);
        logPath("[bolt-ld] sysroot: ", options.sysrootPath);
        logPath("[bolt-ld] runtime root: ", options.runtimeRootPath);
        ...
        logPath("[bolt-ld] entry: ", effectiveEntry);
        logPath("[bolt-ld] map file: ", options.mapFilePath);
